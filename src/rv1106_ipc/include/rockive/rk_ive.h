 ...
 *Modify template filter from 3x3 to 5x5
 */
typedef struct rkIVE_FILTER_AND_CSC_CTRL_S {
...
typedef enum rkIVE_EQUALIZE_MODE_E {
    ...
    IVE_EQUALIZE_MODE_EQHIST_WO_HIST = 0x1,
...
typedef struct rkIVE_GMM_CTRL_S {
    ...
    ive_u8q2 u8q2WeightInitVal;   /*Initial frequency (default: 20000)*/
    ive_u8q2 u8q2WeightAddFactor; /*Frequency adding factor (default: 0xEF)*/
    ive_u8q2 u8q2WeightReduFactor; /*Frequency reduction factor (default: 0xFF00)*/
    ive_u8q2 u8q2WeightThr;   /*Frequency threshold (default: 12000)*/

    RK_U8 u8VarThreshGen;    /*Global sensitivity factor (default: 8)*/
    ive_u8q2 u8q2BgRatio; /*Background ratio*/

    ive_u10q0 u10q0InitVar; /*Initial noise Variance*/
    ive_u10q0 u10q0MinVar;  /*Min variation (default: ( 8 *  8)<<7)*/
    ive_u10q0 u10q0MaxVar;  /*Max variation (default: (16 * 16)<<7)*/
...
typedef struct rkIVE_GMM2_CTRL_S {
    ...
    ive_u8q2 u8q2WeightAddFactor; /*Frequency adding factor (default: 0xEF)*/
    ive_u8q2
        u8q2WeightReduFactor; /*Frequency reduction factor (default: 0xFF00)*/
    ive_u8q2 u8q2WeightThr;   /*Frequency threshold (default: 12000)*/
...
typedef struct rkIVE_GRAD_FG_CTRL_S {
    ...
    RK_U16 u16EdwFactor; /*Edge width adjustment factor (range: 500 to 2000;
                         default: 1000)*/
    RK_U8 u8CrlCoefThr;  /*Gradient vector correlation coefficient threshold
                         (ranges: 50 to 100; default: 80)*/
    RK_U8 u8MagCrlThr;   /*Gradient amplitude threshold (range: 0 to 20; default:
                         4)*/
    RK_U8 u8MinMagDiff;  /*Gradient magnitude difference threshold (range: 2 to 8;
                         default: 2)*/
    RK_U8 u8NoiseVal; /*Gradient amplitude noise threshold (range: 1 to 8; default:
                      1)*/
    RK_U8 u8EdwDark;  /*Black pixels enable flag (range: 0 (no), 1 (yes); default:
                      1)*/
} IVE_GRAD_FG_CTRL_S;

#if 0
/*
 *Candidate background pixel info
 */
typedef struct rkIVE_CANDI_BG_PIX_S {
    RK_U16 u8q4f4Mean;       /*Candidate background grays value */
    RK_U16 u16StartTime;     /*Candidate Background start time */
    RK_U16 u16SumAccessTime; /*Candidate Background cumulative access time */
    RK_U16 u16ShortKeepTime; /*Candidate background short hold time*/
    RK_U8 u8ChgCond; /*Time condition for candidate background into the changing
                     state*/
    RK_U8 u8PotenBgLife; /*Potential background cumulative access time */
} IVE_CANDI_BG_PIX_S;

/*
 *Working background pixel info
 */
typedef struct rkIVE_WORK_BG_PIX_S {
    RK_U16 u8q4f4Mean; /*0# background grays value */
    RK_U16 u16AccTime; /*Background cumulative access time */
    RK_U8 u8PreGray;   /*Gray value of last pixel */
    RK_U8 u5q3DiffThr; /*Differential threshold */
    RK_U8 u8AccFlag;   /*Background access flag */
    RK_U8 u8BgGray[3]; /*1# ~ 3# background grays value */
} IVE_WORK_BG_PIX_S;

/*
 *Backgroud life struct
 */
typedef struct rkIVE_BG_LIFE_S {
    RK_U8 u8WorkBgLife[3]; /*1# ~ 3# background vitality */
    RK_U8 u8CandiBgLife;   /*Candidate background vitality */
} IVE_BG_LIFE_S;
 ...
 */
typedef struct rkIVE_FG_STAT_DATA_S {
    RK_U32 u32PixNum;
    RK_U32 u32SumLum;
    RK_U8 u8Reserved[8];
} IVE_FG_STAT_DATA_S;
...
/*
 *Forgroud statistical data
 */
typedef struct rkIVE_BG_STAT_DATA_S {
    RK_U32 u32PixNum;
    RK_U32 u32SumLum;
    RK_U8 u8Reserved[8];
} IVE_BG_STAT_DATA_S;
...
/*
 *UpdateBgModel control parameters
 */
typedef struct rkIVE_UPDATE_BG_MODEL_CTRL_S {
    RK_U32 u32CurFrmNum;    /*Current frame timestamp, in frame units */
    RK_U32 u32PreChkTime;   /*The last time when background status is checked */
    RK_U32 u32FrmChkPeriod; /*Background status checking period (range: 0 to 2000
                            frames; default: 50) */

    RK_U32 u32InitMinTime; /*Background initialization shortest time (range: 20 to
                           6000 frames; default: 100)*/
    RK_U32 u32StyBgMinBlendTime; /*Steady background integration shortest time
                                 (range: 20 to 6000 frames; default: 200)*/
    RK_U32 u32StyBgMaxBlendTime; /*Steady background integration longest time
                                 (range: 20 to 40000 frames; default: 1500)*/
    RK_U32 u32DynBgMinBlendTime; /*Dynamic background integration shortest time
                                 (range: 0 to 6000 frames; default: 0)*/
    RK_U32 u32StaticDetMinTime;  /*Still detection shortest time (range: 20 to 6000
                                 frames; default: 80)*/
    RK_U16 u16FgMaxFadeTime; /*Foreground disappearing longest time (range: 1 to
                             255 seconds; default: 15)*/
    RK_U16 u16BgMaxFadeTime; /*Background disappearing longest time (range: 1 to
                             255  seconds ; default: 60)*/

    RK_U8 u8StyBgAccTimeRateThr; /*Steady background access time ratio threshold
                                 (range: 10 to 100; default: 80)*/
    RK_U8 u8ChgBgAccTimeRateThr; /*Change background access time ratio threshold
                                 (range: 10 to 100; default: 60)*/
    RK_U8 u8DynBgAccTimeThr;     /*Dynamic background access time ratio threshold
                                 (range: 0 to 50; default: 0)*/
    RK_U8 u8DynBgDepth; /*Dynamic background depth (range: 0 to 3; default: 3)*/
    RK_U8 u8BgEffStaRateThr; /*Background state time ratio threshold when
                             initializing (range: 90 to 100; default: 90)*/

    RK_U8 u8AcceBgLearn;  /*Whether to accelerate background learning (range: 0
                          (no), 1 (yes); default: 0)*/
    RK_U8 u8DetChgRegion; /*Whether to detect change region (range: 0 (no), 1
                          (yes); default: 0)*/
...
typedef struct rkIVE_SAD_CTRL_S {
    ...
    RK_U16 u16Thr;  /*srcVal <= u16Thr, dstVal = minVal; srcVal > u16Thr, dstVal =
                    maxVal.*/
